...
const buildFilterSummary = (filters: ParsedPropertySearchQuery): string => {
  ...
  if (items.length === 0) {
    return "Filtros: sin filtros adicionales";
  }
...
export const buildPropertySearchPdf = async ({
  ...
  results.data.forEach((property, index) => {
    const needsNewPage =
      current.nextY - PROPERTY_CARD_HEIGHT < PAGE_MARGIN_BOTTOM;
    ...
    current.page.drawText(
      toPdfSafeText(`Precio: ${toPriceText(property.price)}   |   Habitaciones: ${property.rooms}`),
      {
        x: PAGE_MARGIN_X + 42,
        y: cardY + 15,
        size: 9,
  ...
  if (results.data.length === 0) {
    ...
    current.page.drawText("No se encontraron propiedades para los filtros seleccionados.", {
      x: PAGE_MARGIN_X + 12,
      y: current.nextY - 24,
      size: 10,
      font: regularFont,
